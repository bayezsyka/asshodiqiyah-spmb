...
export default function Daftar({ jenjang, periode, persyaratan }: { jenjang: Jenjang[]; periode: { id: number; nama: string; tahun_ajaran: string; informasi?: string; instruksi_pembayaran?: string } | null; persyaratan: Persyaratan[] }) {
  ...
  const berkasDiminta = useMemo(() => {
    ...
    const sesuaiKonteks = persyaratan.filter((item) => {
      ...
      if (isPaud) {
        return sesuaiJenjang && !item.periode_ppdb_id;
      }
  ...
  const field = ([nama, label]: string[]) => {
    ...
    const wajib = bidangWajib.has(nama) || (nama === 'nisn' && terpilih?.kelompok !== 'paud') || (nama === 'nik' && terpilih?.kelompok === 'paud');
    const batasDigit = nama === 'nisn' ? 10 : nama === 'nik' ? 16 : 24;
    return <label key={nama}><span className="spmb-label">{label}{wajib && <span className="text-red-600"> *</span>}</span><input className="spmb-input" type={tipe} inputMode={angkaBulat || hanyaDigit ? 'numeric' : angkaDesimal ? 'decimal' : undefined} min={angkaBulat || angkaDesimal ? 0 : undefined} step={angkaDesimal ? '0.01' : angkaBulat ? 1 : undefined} maxLength={hanyaDigit ? batasDigit : undefined} required={wajib} value={form.data[nama] ?? ''} onChange={(event) => ubahNilai(nama, hanyaDigit ? event.target.value.replace(/\D/g, '') : event.target.value)}/>{nama === 'nisn' && <p className="spmb-help">Wajib untuk SD, SMP, dan SMA.</p>}{nama === 'nik' && <p className="spmb-help">Wajib untuk PAUD. Untuk jenjang lain dapat diisi bila tersedia.</p>}{form.errors[nama] && <p className="spmb-error">{form.errors[nama]}</p>}</label>;
  };
  const pilihJenjang = (id: string) => {
    const pilihan = jenjang.find((item) => item.id === Number(id));
    form.setData({ ...form.data, jenjang_pendaftaran_id: Number(id), periode_ppdb_id: pilihan?.kelompok === 'paud' ? '' : (periode?.id ?? ''), nisn: pilihan?.kelompok === 'paud' ? '' : form.data.nisn });
  };
    ...
    <section className="flex-1 py-10 sm:py-16 lg:py-20"><div className="spmb-container">
      ...
      <div className="mx-auto mt-10 max-w-5xl">
        ...
        <form id="formulir-pendaftaran" className="mt-8 rounded-[1.25rem] border border-slate-200 bg-white p-5 shadow-[0_1.25rem_3rem_rgba(3,41,68,.06)] sm:p-8" onSubmit={(event) => { event.preventDefault(); kirimFormulir(); }}>
          ...
          {langkahAktif === 1 && <div className="mx-auto max-w-2xl"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-primary"><Info className="h-5 w-5"/></span><div><h2 className="text-xl font-bold text-ink">Pilih jenjang tujuan</h2><p className="mt-1 text-sm leading-6 text-slate-600">Formulir mengikuti data pada formulir resmi Lenterahati IBS.</p></div></div><label className="mt-7 block"><span className="spmb-label">Jenjang pendidikan</span><select className="spmb-input" value={form.data.jenjang_pendaftaran_id} onChange={(event) => pilihJenjang(event.target.value)}><option value="">Pilih jenjang</option>{jenjang.map((item) => <option key={item.id} value={item.id}>{item.nama}</option>)}</select>{form.errors.jenjang_pendaftaran_id && <p className="spmb-error">{form.errors.jenjang_pendaftaran_id}</p>}</label>{terpilih && <div className="mt-5 rounded-xl bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-600"><strong className="text-ink">{terpilih.nama}</strong>{terpilih.kelompok === 'paud' ? ' · Pendaftaran dibuka sepanjang tahun (tanpa periode gelombang).' : ` · ${periode ? `Periode ${periode.nama} (${periode.tahun_ajaran}).` : 'Periode pendaftaran formal belum dibuka oleh panitia.'}`}</div>}{!periode && terpilih && terpilih.kelompok !== 'paud' && <p className="mt-3 text-xs font-semibold text-red-600">Periode SPMB untuk jenjang ini belum aktif. Silakan hubungi panitia atau pantau informasi terbaru.</p>}<div className="mt-8 flex justify-end"><button type="button" disabled={!terpilih || (!periode && terpilih.kelompok !== 'paud')} className="spmb-button-primary" onClick={berikutnya}>Lanjutkan <ChevronRight className="h-4 w-4"/></button></div></div>}
